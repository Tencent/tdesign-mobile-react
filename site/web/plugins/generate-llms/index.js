...
/** 列出 demo 的候选路径：_example/<demoName>.tsx（含 kebab/camelCase 双写）与 _example/<demoName>/index.tsx。 */
function demoCandidates(componentDir, demoName) {
  ...
  return names.flatMap((name) => [
    path.join(componentDir, '_example', `${name}.tsx`),
    path.join(componentDir, '_example', name, 'index.tsx'),
  ]);
}

/**
 * 读取组件目录下的 demo 源码：_example/<demoName>.tsx，回退 _example/<demoName>/index.tsx。
 * 输出 TSX 代码块。
 */
function readTsxDemo(componentDir, demoName) {
 ...
 * 此处仅负责 vite 构建钩子分发，并按 mobile-react 仓库约定注入组件文档与 demo 源码读取器。
 */
  ...
  return {
    ...
    async closeBundle(error) {
      ...
      await generateLlmsDocs({
        ...
        platform: 'mobile',
        parseComponentDoc,
        siteTitle: 'TDesign Mobile React',
        siteDescription: 'TDesign Mobile React 组件库的 LLM 友好文档索引。',
