...
export default function useColumnResize(params: {
  ...
  const getMinMaxColWidth = (targetCol: BaseTableCol<TableRowData>) => {
    ...
    return {
      minColWidth: Math.max(DEFAULT_MIN_WIDTH, propMinWidth),
      maxColWidth: DEFAULT_MAX_WIDTH,
    };
  };
  ...
  const onColumnMousedown = (
    ...
    const onDragEnd = () => {
      ...
      const tmpCurrentCol = currentSibling;
  ...
  const updateTableWidthOnColumnChange = (colKeys: string[]) => {
    ...
    const oldTotalWidth = Object.values(thWidthList).reduce((r, n) => r + n, 0);
