...
describe('ActionSheet', () => {
  ...
  describe('props', () => {
    ...
    it(':cancelText - should display custom cancel text', () => {
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} showCancel={true} cancelText="Custom Cancel" />
      );
    ...
    it(':description - should render description', () => {
      const { container } = render(
        <ActionSheet {...defaultProps} description="Test description" />
      );
    ...
    it(':description - should apply left alignment class', () => {
      const { container } = render(
        <ActionSheet {...defaultProps} description="Test" align="left" />
      );
    ...
    it(':description - should apply grid theme class', () => {
      const { container } = render(
        <ActionSheet {...defaultProps} description="Test" theme="grid" />
      );
    ...
    it(':popupProps - should handle popup props', () => {
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} popupProps={{ placement: 'top' }} />
      );
      expect(getByTestId('popup')).toBeInTheDocument();
    });
  });
  ...
  describe('events', () => {
    it(':onSelected - should call onSelected when list item is selected', () => {
      ...
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} theme="list" onSelected={onSelected} />
      );
    ...
    it(':onSelected - should call onSelected when grid item is selected', () => {
      const onSelected = vi.fn();
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} theme="grid" onSelected={onSelected} />
      );
    ...
    it(':onCancel - should call onCancel when cancel button is clicked', () => {
      ...
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} showCancel={true} onCancel={onCancel} />
      );
    ...
    it('should handle popup onVisibleChange', () => {
      const onClose = vi.fn();
      const { getByTestId } = render(
        <ActionSheet {...defaultProps} onClose={onClose} />
      );
  ...
  describe('edge cases', () => {
    ...
    it('should handle missing onSelected callback', () => {
      const { getByTestId } = render(
        <ActionSheet visible={true} items={defaultProps.items} theme="list" />
      );
    ...
    it('should handle missing onCancel callback', () => {
      const { getByTestId } = render(
        <ActionSheet visible={true} items={defaultProps.items} showCancel={true} />
      );
    ...
    it('should handle missing onClose callback', () => {
      const { rerender } = render(
        <ActionSheet visible={true} items={defaultProps.items} />
      );
  ...
  describe('component behavior', () => {
    it('should handle visibility changes', () => {
      const { rerender, getByTestId, queryByTestId } = render(
        <ActionSheet {...defaultProps} visible={true} />
      );
      expect(getByTestId('popup')).toBeInTheDocument();
    ...
    it('should render footer with gap class for list theme', () => {
      const { container } = render(
        <ActionSheet {...defaultProps} theme="list" showCancel={true} />
      );
    ...
    it('should render footer with gap class for grid theme', () => {
      const { container } = render(
        <ActionSheet {...defaultProps} theme="grid" showCancel={true} />
      );
  ...
  describe('default props', () => {
    ...
    it('should handle minimal props', () => {
      const { getByTestId } = render(
        <ActionSheet visible={true} items={['Option 1']} theme="list" />
      );
      expect(getByTestId('popup')).toBeInTheDocument();
      expect(getByTestId('action-sheet-list')).toBeInTheDocument();
    });
